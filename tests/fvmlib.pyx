...
def VF_markers_update_PIPC(
    ...
    cdef np.ndarray[dtype_t, ndim=1] a, b, p # point A, B, P
    cdef np.ndarray[dtype_t, ndim=1] ind_v # induced velocity
    cdef np.ndarray[dtype_t, ndim=4] pos_markers_predictor

    pos_markers_predictor = np.copy(pos_markers)
    ...
    for index_m in range(N_m):
        for index_r in range(N_r):
            for index_b in range(N_b):
                p = pos_markers_predictor[index_m, index_r, index_b] # update this marker
                ind_v = np.zeros(3)
                for i in range(N_m-1):
                    for j in range(N_r):
                        for k in range(N_b):
                            if dir_psi[j] < 0: # clockwise
                                a = pos_markers_predictor[i, j, k] # point A of this segment
                                b = pos_markers_predictor[i+1, j, k] # point B of this segment
                            else: # counter-clockwise
